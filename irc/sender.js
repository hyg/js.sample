...
const config = {
  ...
  port: 6667,                    // IRC 服务器端口
  nick: 'qwen_sender_' + Math.random().toString(36).substr(2, 5), // 发送方昵称
...
// 将日志写入文件
const logFile = fs.createWriteStream('irc_sender.log', { flags: 'w' });
...
client.addListener('registered', (message) => {
  ...
  setTimeout(() => {
    ...
    client.say(config.channels[0], testMessage);
  }, 5000); // 等待5秒确保加入频道
});
...
client.addListener('message', (from, to, text, message) => {
  ...
  // 如果收到对方的消息，就回复
  if (from.startsWith('qwen_receiver_')) {
