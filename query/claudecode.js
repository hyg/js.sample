...
function processPeriodBalances(year, month) {
    ...
    for (let y = 2020; y <= year; y++) {
        ...
        for (let m = 1; m <= monthsInYear; m++) {
            const periodAccounts = processAERData(y, m);
            openingAccounts = periodAccounts;
        }
    }
    
    // Get closing balance (end of current period)
    const closingAccounts = processAERData(year, month);
...
function generateBalanceSheet(accounts) {
    ...
    // Categorize accounts with hierarchical aggregation
    Object.entries(accounts).forEach(([key, account]) => {
        const balance = account.balance.rmb;
        
        // Only include parent accounts and leaf accounts, not intermediate levels
        if (key === '总账') return; // Skip root
        
        // Assets
        if (key === '银行存款' || key === '现金' || key === '微信零钱' || key === 'raw') {
            assets[key] = { ...account, balance: Math.abs(balance) };
        }
        // Equity
        else if (key === 'donation' || key === 'ego' || key === 'PSMD' || key === 'xuemen') {
            equity[key] = { ...account, balance: Math.abs(balance) };
        }
    });
...
function generateIncomeStatement(accounts) {
    ...
    Object.entries(accounts).forEach(([key, account]) => {
        if (key === 'donation') {
            revenue[key] = { ...account, amount: Math.abs(account.balance.rmb) };
        }
    });
    ...
    Object.entries(accounts).forEach(([key, account]) => {
        if (key === 'raw') {
            expenses[key] = { ...account, amount: Math.abs(account.balance.rmb) };
        }
    });
        ...
        Object.entries(chartOfAccounts).forEach(([key, account]) => {
            if (key !== '总账') { // Skip root account
                ...
                if (Math.abs(openingBalance) > 0.01 || Math.abs(closingBalance) > 0.01 || Math.abs(change) > 0.01) {
                    console.log(`${account.name}\t${openingBalance.toFixed(2)}\t${closingBalance.toFixed(2)}\t${change.toFixed(2)}`);
                }
            }
        });
        console.log("");
...
function displayFinancialStatementsForPeriod(statements) {
    ...
    console.log(`资产总计: ${statements.balanceSheet.assets.total.toFixed(2)}`);
    
    console.log("\n权益:");
    Object.entries(statements.balanceSheet.equity.accounts).forEach(([key, acc]) => {
        console.log(`  ${acc.name}: ${acc.balance.toFixed(2)}`);
    });
    console.log(`权益总计: ${statements.balanceSheet.equity.total.toFixed(2)}`);
    ...
    Object.entries(statements.incomeStatement.revenue.accounts).forEach(([key, acc]) => {
        console.log(`  ${acc.name}: ${acc.amount.toFixed(2)}`);
    });
    console.log(`收入合计: ${statements.incomeStatement.revenue.total.toFixed(2)}`);
    
    console.log("\n费用:");
    Object.entries(statements.incomeStatement.expenses.accounts).forEach(([key, acc]) => {
        console.log(`  ${acc.name}: ${acc.amount.toFixed(2)}`);
    });
    console.log(`费用合计: ${statements.incomeStatement.expenses.total.toFixed(2)}`);
    ...
    Object.entries(statements.cashFlowStatement.operatingActivities.accounts).forEach(([key, acc]) => {
        console.log(`  ${acc.name}: ${acc.cashFlow.toFixed(2)}`);
    });
    console.log(`经营活动产生的现金流量净额: ${statements.cashFlowStatement.operatingActivities.netCashFlow.toFixed(2)}`);
...
allFinancialStatements.forEach(statements => {
    ...
    Object.entries(chartOfAccounts).forEach(([key, account]) => {
        if (key !== '总账') {
            ...
            const closingBalance = statements.period.closing[key] ? statements.period.closing[key].balance.rmb : 0;
            const change = closingBalance - openingBalance;
